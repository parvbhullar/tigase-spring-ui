...
public class ReportService {
    ...
    public void txAddOrEditReport(Map<String, Object> params, Long memberId) {
        ...
        if (reportId != null) {
            ...
            // 对新增广告进行特别处理，如果新增广告包边没有广告组节点，则给其加上
            // wangxing 2011-09-22
            String tempId = report.getReportCon();
            if (StringUtils.isNotBlank(tempId) && tempId.split(";").length == 1) {
                if (tempId.indexOf("a") >= 0) {
                    tempId = tempId.replaceAll("a", "");
                    CoreAd ad = adService.findCoreAdByAdId(Integer.valueOf(tempId), memberId);
                    StringBuilder sb = new StringBuilder();
                    if (ad.getAdGroupId() != null) {
                        sb.append("g").append(String.valueOf(ad.getAdGroupId())).append(";").append(
                                report.getReportCon());
                    }
                    report.setReportCon(sb.toString());
                }
            }
