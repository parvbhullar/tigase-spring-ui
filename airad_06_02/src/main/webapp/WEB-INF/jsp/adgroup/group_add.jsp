...
<%
    CoreCampaign coreCampaign = (CoreCampaign) request
          .getAttribute("campaign");
      String startTime = "";
      if (null != coreCampaign) {
        if (null != coreCampaign.getStartTime()) {
          startTime = DateFormatUtils.format(coreCampaign
...
<small style="display:block">请输入一个有助于您识别该广告组的名称，比如：长三角地区人群</small></td>
    </tr>
    <tr>
      <th><span class="must">*</span>地理位置</th>
      <td><form:radiobutton path="coreAdGroup.adLoclType" value="0"
        onclick="showDetial();" checked="checked" />全国
      <form:radiobutton path="coreAdGroup.adLoclType" value="2"
        onclick="showDetial();" /> 精确到区 <form:radiobutton
        path="coreAdGroup.adLoclType" value="1"
      ...
      <div style="height: 150px;width:460px; overflow: auto; display: none"
        class="selectBox" id="proId">
        <c:forEach items="${form.proListBean}" var="DictionaryGlobalRegion" varStatus="statu">
        <div>
  ...
  <tr>
   ...
   -->
    <tr>
      ...
      <td>
      <div class="btnBox">
       <button class="btnBY fl" type="button" onclick="registerAdGroupPageSubmti();">提交</button>
      <div class="moreBtn"><span class="gray">|</span> <a
        href="javascript:returnPage();">取消</a></div>
      </div>
      </td>
    </tr>
  </table>
...
<ul>
  <li><span class="fr"><airad:cutString size="10" value="${campaign.campaignName}" mark="..."/></span>名称</li>
...
<ul>
  <li>
  <span id="adGroupNamesDiv" class="fr"></span>名称
  </li>
  <li>
