...
Ext.onReady(function() {
	...
	var store = new Ext.data.Store({
				proxy : new Ext.data.HttpProxy({
							url : './param.xzb?reqCode=queryParamsForManage'
						}),
	...
	function saveParamItem() {
		...
		addParamFormPanel.form.submit({
					url : './param.xzb?reqCode=saveParamItem',
					waitTitle : '提示',
					method : 'POST',
					waitMsg : '正在处理数据,请稍候...',
	...
	function synMemory(msg, flag) {
		Ext.Msg.confirm('请确认', msg, function(btn, text) {
					if (btn == 'yes') {
						showWaitMsg();
						Ext.Ajax.request({
									url : 'param.xzb?reqCode=synMemory',
	...
	function deleteParamItems() {
		...
		Ext.Msg.confirm('请确认', '确认删除选中的全局参数吗?', function(btn, text) {
					if (btn == 'yes') {
						...
						showWaitMsg();
						Ext.Ajax.request({
									url : './param.xzb?reqCode=deleteParamItems',
									success : function(response) {
	...
	function update() {
		addParamFormPanel.form.submit({
					url : './param.xzb?reqCode=updateParamItem',
					waitTitle : '提示',
					method : 'POST',
					waitMsg : '正在处理数据,请稍候...',
					success : function(form, action) {
