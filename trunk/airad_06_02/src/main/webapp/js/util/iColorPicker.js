            ...
            for (k = 0; k < tblCells.length; k++) {
                jQuery(tblChilds[j].childNodes[k]).unbind().mouseover(function(a){
                    ...
                    jQuery("#iColorPicker").fadeOut();
					 $("#bannerDemo").css("background-image","url('')"); 
					jQuery("#bannerDemo").css("background-color",aaa);
                    jQuery(this)
                })
            }
        }
...
function iColorPicker(){
    jQuery("input.iColorPicker").each(function(i){
        ...
        if(jQuery(this).val().trim()==""){jQuery(this).val("#00f")}
        try{
        jQuery(this).css("backgroundColor", jQuery(this).val().trim()).after('<a href="javascript:void(null)" id="icp_' + this.id + '" onclick="iColorShow(\'' + this.id + '\',\'icp_' + this.id + '\')" class="collapsed"><img src="' + imageUrl + '" style="border:0;margin:0" align="absmiddle" />选取背景颜色</a>')
       }catch(e){
           // alert(e)           
       }
    })
