...
if(bannerHtml!="")
{
  ...
  if (bt == 9) {
	...
	$bannerOut.attr("contenteditable",true);
	   //原始banner值，提交时判断用户是否修改了banner
    $("#adBannerContent").val($bannerOut.text());
...
if($("#adType").val()==2||$("#adType").val()==3||$("#adType").val()==4){ 
    ...
    <c:forEach items="${command.richList}" var="rich" varStatus="vai">
      ...
      $("#showTypeOrignal").val("${rich.showType}");
      ...
      if("${rich.showType}"=="9")
      {
        changeChildPage("9","showType"+"${rich.sort}");
        var rel = "${rich.relId}";
        $("#showType"+"${rich.sort}"+"CallPhoneNumber").val("${rich.libCallPhone.callPhoneNumber}");
        $("#showType"+"${rich.sort}"+"CallPhoneId").val("${rich.libCallPhone.id}");
      }
