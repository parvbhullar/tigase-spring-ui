...
<div><c:if test="${command.memberMessage=='inviteCodeNullity'}">
	...
	<ul>
		...
		<li>从这里<a href="javascript:jumpPage('member.do?action=logon&loginType=${command.loginType }&email=${command.email }')">进入登录页。</a></li>
	</ul>
	<script>
        editDivStyle();
      </script>
 ...
 <c:if test="${command.memberMessage=='passwordResult'}">
		...
		<h2>密码重置成功</h2>
      <ul>
			<li><a href="javascript:jumpPage('member.do?action=logon&loginType=${command.loginType }&email=${command.email }')"><strong>返回登录</strong></a>。</li>
			</ul>
		</c:if>
		<c:forEach items="${command.errors}" var="errorinfo">
		<h2>密码重置失败</h2>
      ...
      <ul>
			...
			<li><a href="javascript:jumpPage('member.do?action=logon&loginType=${command.loginType }&email=${command.email }')">返回登录</a></li>
...
<c:if test="${command.memberMessage=='logonRegister'}">
	...
	<ul>
		...
		<li><a href="javascript:jumpPage('member.do?action=logout&loginType=${command.loginType }&email=${command.email }')">退出登录</a>。
 ...
 <c:if test="${command.memberMessage=='sendEmail'}">
	<c:if test="${fn:length(command.errors)<1 }">
		<h2>邮件发送成功</h2>
		<ul>
			<li><a href="javascript:jumpPage('member.do?action=logon&loginType=${command.loginType }&email=${command.email }')">现在登录</a></li>
		</ul>
	</c:if>
	<c:forEach items="${command.errors}" var="errorinfo">
		<h2>邮件发送失败</h2>
		...
		<ul>
			...
			<c:choose>
				<c:when test="${errorinfo.value=='每天至多发送3封密码重置邮件' }">
					<li><a href='javascript:jumpPage("/member.do?action=logon&loginType=${command.loginType }&email=${command.email }")'>返回登录页面</a>。</li>
				</c:when>
				<c:otherwise>
	...
	<c:if test="${fn:length(command.errors)<1 }">
		<h2>激活成功</h2>
		<ul>
			<li><a href="javascript:jumpPage('member.do?action=logon&loginType=${command.loginType }&email=${command.email }')">立刻登录</a></li>
		</ul>
	</c:if>
	<c:forEach items="${command.errors}" var="errorinfo">
		<h2>激活失败</h2>
