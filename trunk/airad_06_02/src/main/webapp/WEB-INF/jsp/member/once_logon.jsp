...
<form:form name="myFrm" commandName="command" action="member.do?action=doLogon"
  ...
  <table border="0" cellspacing="0" cellpadding="0" class="tabNF reg">
    ...
    <tr>
      ...
      <td><div class="fl"><c:out value="${command.email}"></c:out></div><div class="fTip">该邮箱已注册，输入登录密码完成绑定</div><form:hidden path="email" cssClass="half" autocomplete="off" />
      </td>
           </tr>
    <tr>
      <th>密码</th>
  ...
  <c:when test="${not empty command.errors }">
     ...
     <div class="moreBtn"><span class="gray">|</span>
       <a href="member.do?action=sendResettingPasswordEmailPage&loginType=1&email=${command.email}">找回密码</a></div>
       </div></td>
    </tr>
