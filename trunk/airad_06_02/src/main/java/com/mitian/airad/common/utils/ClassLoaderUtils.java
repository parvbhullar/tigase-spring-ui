...
import com.mitian.airad.common.utils.io.StreamUtils;
...
public abstract class ClassLoaderUtils {
    ...
    public static Class loadServiceClass(String serviceId, ClassLoader classLoader) throws ClassNotFoundException {
        ...
        try {
            serviceClassName = StringUtils.trimToEmpty(StreamUtils.readText(istream, "UTF-8"));
        }
        catch (IOException e) {
    ...
    public static Class loadServiceClass(String className, String serviceId, ClassLoader classLoader)
        ...
        try {
            ...
            LOGGER.error("类[" + className + "]加载异常" + e.getMessage());
        }
    ...
    public static <I> I getInstanceByInterface(Class<I> interfaceDefinition, String defaultImplClass, boolean needCache) {
        return getInstanceByInterface(interfaceDefinition, getContextClassLoader(), defaultImplClass, needCache);

    }

    public static <I> I getInstanceByInterface(Class<I> interfaceDefinition, ClassLoader classLoader,
            String defaultImplClass, boolean needCache) {

        I result = null;

        if (needCache && localConfig.get(interfaceDefinition.getName()) != null) {
            return newInstance(interfaceDefinition, localConfig.get(interfaceDefinition.getName()).toString(),
                    classLoader);
        }
    ...
    public static <I> I newInstance(Class<I> interfaceDefinition, String className, ClassLoader classLoader) {
        try {
            Class<I> spiClass;
    ...
    private static boolean getResources(List urlSet, String resourceName, ClassLoader classLoader,
        ...
        try {
            ...
            LOGGER.error("文件[" + resourceName + "]加载异常" + e.getMessage());
        }
    ...
    private static URL[] getDistinctURLs(LinkedList urls) {
        ...
        Set<URI> uriSet = new HashSet<URI>(urls.size());

        try {
            for (Iterator i = urls.iterator(); i.hasNext();) {
                URL url = (URL) i.next();

                if (uriSet.contains(url.toURI())) {
                    i.remove();
                }
                else {
            ...
            LOGGER.error("URL转URI报错" + e.getMessage());
