...
public class AdService implements ConstantAd {
    Logger log = Logger.getLogger(AdService.class);
    /** spring注入CoreAgentRelationDAO */
    @Autowired
    public CoreAdDAO coreAdDAO;
    @Autowired
    private CoreBannerDAO coreBannerDAO;
    @Autowired
    public CoreWapAdDAO coreWapAdDAO;
    @Autowired
    public CoreRichAdDAO coreRichAdDAO;
    @Autowired
    public LibMapInfoDAO libMapInfoDAO;
    @Autowired
    public LibPointInfoDAO libPointInfoDAO;
    @Autowired
    public CoreRichGuideDAO coreRichGuideDAO;
    @Autowired
    ...
    public List<CoreAd> queryList4Shopper(Long memberId, int page, String sortName, String sortType, String searchAdName) {
        List<CoreAd> list = new ArrayList<CoreAd>();
        CoreAd coreAd = new CoreAd();
    ...
    public int countTotal(Integer adGroupId, String adName, String flag, Long memberId) {
        ...
        int adSize = 0;
        if (StringUtils.isNotBlank(adGroupId.toString())) {
            // adSize = coreAdDAO.countTotal(params);
        }
        return adSize;
    }
