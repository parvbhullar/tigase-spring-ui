...
import com.mitian.airad.common.utils.i18n.LocaleUtils;
import com.mitian.airad.common.utils.internal.Entities;
...
public class StringEscapeUtils {
    ...
    public static String formatHtml(String source, boolean bReplaceNl, boolean bReplaceTag, boolean bReplaceQuote) {
        return formatHtml(source, bReplaceNl, bReplaceTag, bReplaceQuote, true);
    }
...
/**
     ...
     * @param bReplaceAmp if true '\&' will be replaced by &amp
     */
    public static String formatHtml(String source, boolean bReplaceNl, boolean bReplaceTag, boolean bReplaceQuote,
            boolean bReplaceAmp) {
     ...
     * @return
     */
    public static String setFeatReturnAdviseForHtm(String advise) {
        if (StringUtils.isNotBlank(advise)) {
            // 兼容原有的存在<br>处理的数据
            if ((advise.indexOf("<br>") > 0) || (advise.indexOf("<BR>") > 0)) {
                return advise;
            }
            else {
                advise = formatHtml(advise, true, false, false, false);
            }
        }
        return advise;
    }
    ...
    /**
     * 处理繁体输入简体字,在后台显示乱码的问题.主要是将&amp;#1233;转为&#1233.
     * 
     * @param str
     * @return
     */
    public static String disposeAmp(String str) {
        // 将&amp;#替换成&#,处理在繁体版输入简体，会出现如&#2342的码字的问题.
        // 只有在&amp;#[0-9]+;这种情况下，认为是繁体输入简体字转为&#
        StringBuffer newsb = new StringBuffer();
        Pattern pt = Pattern.compile("&amp;#[0-9]+;");
        Matcher m = pt.matcher(str);
        int bstart = 0, bend = 0;
        while (m.find()) {
            newsb.append(str.substring(bend, m.start()));
            newsb.append(m.group().replaceAll("&amp;#", "&#"));
            bstart = m.start();
            bend = m.end();
        }
        newsb.append(str.substring(bend));
     ...
     */
    public static String transformCnToAscii(String str) throws IllegalAccessException, InvocationTargetException {
        StringBuffer string = new StringBuffer();
        for (int i = 0; i < str.length(); i++) {
            int abc = str.charAt(i);
            string.append("&#" + abc + ";");
        }
        str = string.toString();
        return str;
    ...
    private static boolean escapeURLInternal(String str, String encoding, Writer out, boolean strict)
            throws IOException {
        if (encoding == null) {
            encoding = LocaleUtils.getContext().getCharset();
        }
