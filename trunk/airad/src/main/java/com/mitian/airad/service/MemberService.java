...
public class MemberService {
    ...
    public Map<String, String> txAddMember(CoreMemberInfo coreMember) {
        ...
        if (!StringUtil.isEmpty(coreMember.getInviteCode())) {
            ...
            coreMemberInfoDAO.insertSelective(coreMember);
        }
        // 发送邮件给用户
    ...
    public String regSuccessSendMail(CoreMemberInfo newRecord, String urlName) {
        ...
        // 测试激活链接
        model.put("activeUrl", urlName + "/member.do?action=regActive&email=" + newRecord.getEmail() + "&activeCode="
                + newRecord.getActiveCode());
        // 调用发送邮件接口发送邮件
