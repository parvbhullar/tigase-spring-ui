...
public class AccMailNoticeService {
    ...
    private void checkNoticeBlanceAndResetIfNeed(AccMailNotice amn) {
        ...
        // 服务器很诡异的在此处抛异常，添加log方便调试
        logger.error("<<<<<<<<<<<<<<<<<<<< accountBlance:" + amn.getAccountBlance() + ">>>>>>>>>>>>>>>>>>>>>>>>>");
        logger.error("<<<<<<<<<<<<<<<<<<<< before exec: SysConfigRefreshbale size:"
                + sysConfigRefreshable.getCacheSize() + "=== isNeedRefresh? " + sysConfigRefreshable.isNeedRefresh()
                + " >>>>>>>>>>>>>>>>>>>>>");
        // bug 477 测试用，邪恶的使缓存失效
        ...
        logger.error("<<<<<<<<<<<<<<<<<<<< after exec: SysConfigRefreshbale size:"
                + sysConfigRefreshable.getCacheSize() + "=== isNeedRefresh? " + sysConfigRefreshable.isNeedRefresh()
                + " >>>>>>>>>>>>>>>>>>>>>");
        if (config != null) {
