...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  <body onLoad="addSelect('s1','s2','endTime','endHour','endMin');">
  <%@ include file="/WEB-INF/jspf/header.jsp" %>
  <%@ include file="/WEB-INF/jspf/calendar.jsp"%>
  <div id="main">
    <div class="mainCon">
 <!-- 程序开始 -->
<h1><img src="images/ico_act.gif" alt="活动" width="16" height="16" align="absmiddle" /><airad:cutString size="10" value="${form.campaign.campaignName}" mark="..."/></h1>
  <div class="leftCon">
  <h1>编辑广告活动</h1>
<form:form action="campaign.do?action=doUpdate&currentPage=${form.currentPage}" method="post" commandName="command">
<table border="0" cellspacing="0" cellpadding="0" class="tabNF">
        ...
        <tr>
          ...
          <td><form:input path="campaign.campaignName"  cssClass="long" onkeyup="onPreview()" maxlength="50"/>
        ...
        <tr>
          ...
          <form:select path="startHour" onchange="onPreview()" >
            <c:forEach items="${form.hourList}" var="hour">
            <form:option value="${hour}" label="${hour}"/>
            </c:forEach>
            </form:select>
            &nbsp;时
            <form:select path="startMin" onchange="onPreview()">
        ...
        <tr>
          <th>是否有结束时间</th>
          <td>
           <form:select path="timeFlag" onChange="showEndTime('endTime','endHour','endMin');">
        ...
        <td>
          <form:input 
           path="endTime" class="cal" onblur="showDigest();"/>
          <form:select path="endHour" onchange="showDigest();">
            ...
            &nbsp;时
            <form:select path="endMin" onchange="showDigest();">
            <form:option value="0" label="0"/>
            <form:option value="15" label="15"/>
            <form:option value="30" label="30"/>
            <form:option value="45" label="45"/>
        ...
        <tr>
          ...
          <td>
            <form:select path="campaign.pubKind">
       ...
       -->
        <tr>
          ...
          <td><sup>&yen;</sup> <form:input path="buggetDay"  cssClass="tiny" onkeyup="onPreview()" maxlength="8"/>
           ${el:errorTip(command.errors,"buggetDay") }
<small style="display:block">此项可有效控制每日广告费用。当每日广告花费达到填入金额时，该活动中的广告将暂停投放，至次日再继续。但预算额度最低不可低于¥50。</small></td>
        </tr>
        <tr>
          <th>活动说明</th>
        ...
        <tr>
          ...
          <div class="btnBox">
           ...
           <div class="moreBtn"><span class="gray">|</span> <a href="campaign.do?action=listCampaign&currentPage=${form.currentPage}">取消</a></div>
           </div>
          </td>
        </tr>
      </table>
...
<li style="display: none" id="showEndTimeIdPreview"><span id="campaign_endTime" class="fr sml"></span>结束时间</li>
