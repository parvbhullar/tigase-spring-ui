...
<%String info=(String)request.getSession().getAttribute("savereport"); %>
<head>
<title>报告列表</title>
</head>
<body>
...
<!-- 开发嵌入start-->
 <c:if test='${p.coreMemberInfo.role.agents}'>
		     <a href="/report.do?action=add&flag=2&currentPage=${p.currentPage}" class="btnY fr"><span>新建报表</span></a>
		    </c:if>
		    <c:if test='${p.coreMemberInfo.role.dev}'>
		       <a href="/report.do?action=add&flag=1&currentPage=${p.currentPage}"class="btnY fr"><span>新建报表</span></a>
        </c:if>
        <c:if test='${p.coreMemberInfo.role.advAndDev}'>
          <c:choose>
           <c:when test="${isCurrentDevRole}">
             <a href="/report.do?action=add&flag=1&currentPage=${p.currentPage}"class="btnY fr"><span>新建报表</span></a>
           </c:when>
           <c:otherwise>
           <a href="/report.do?action=add&flag=0&currentPage=${p.currentPage}"class="btnY fr"><span>新建报表</span></a>
           </c:otherwise>
         </c:choose>
        </c:if>
        <c:if test='${p.coreMemberInfo.role.advertisers || p.coreMemberInfo.role.general}'>
         <a href="/report.do?action=add&flag=0&currentPage=${p.currentPage}"class="btnY fr"><span>新建报表</span></a>
        </c:if>
...
<div class="ctl"><a href="#" onclick="deleteReport();" class="btnS f1"><span>删除</span></a></div>
</c:if>
	<table cellspacing="0" cellpadding="0" border="0" class="tabY">
		...
		<tbody>
			...
			<c:forEach items="${p.listBean}" var="report" varStatus="s">
				<tr>
					<td><form:checkbox path="deleteList" value="${report.reportId}" />
            <a href="report.do?action=show&reportId=${report.reportId}">
            <airad:cutString size="20" value="${report.reportName }" mark="..."/></a></td>
