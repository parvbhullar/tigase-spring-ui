...
Ext.onReady(function() {
	...
	var store = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'otherDemo.xzb?reqCode=queryFileDatas'
		}),
	...
	grid.on("cellclick", function(pGrid, rowIndex, columnIndex, e) {
		...
		if (fieldName == 'download' && columnIndex == 2) {
			...
			downloadIframe.src = 'otherDemo.xzb?reqCode=downloadFile&fileid=' + fileid;
	...
	function submitTheForm() {
		...
		var requesturl = 'otherDemo.xzb?reqCode=doUpload';
		if(mode == 'ftp') {
		   requesturl = 'otherDemo.xzb?reqCode=doUploadByFTP';
		}
	...
	function delFiles() {
		...
		Ext.Msg.confirm('请确认', '你真的要删除选中的文件吗?', function(btn, text) {
			if (btn == 'yes') {
				showWaitMsg();
				Ext.Ajax.request({
					url : 'otherDemo.xzb?reqCode=delFiles',
