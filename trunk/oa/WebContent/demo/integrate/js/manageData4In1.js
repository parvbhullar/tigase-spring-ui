...
Ext.onReady(function() {
	...
	var store = new Ext.data.Store({
				...
				proxy : new Ext.data.HttpProxy({
							url : 'integrateDemo.xzb?reqCode=querySfxmDatas'
						}),
	...
	function submitTheForm() {
		if (!myForm.getForm().isValid())
			return;
		myForm.form.submit({
					url : 'integrateDemo.xzb?reqCode=updateSfxm',
					waitTitle : '提示',
					method : 'POST',
					waitMsg : '正在处理数据,请稍候...',
	...
	function submitTheFormBasedBatch() {
		...
		myForm.form.submit({
					url : 'integrateDemo.xzb?reqCode=batchSql',
					waitTitle : '提示',
					method : 'POST',
					waitMsg : '正在处理数据,请稍候...',
	...
	function submitTheForm() {
		if (!myForm.getForm().isValid())
			return;
		myForm.form.submit({
					url : 'integrateDemo.xzb?reqCode=saveSfxmDomain',
					waitTitle : '提示',
					method : 'POST',
					waitMsg : '正在处理数据,请稍候...',
					success : function(form, action) { // 回调函数有2个参数
	...
	function deleteCatalogItem() {
		...
		Ext.MessageBox.confirm('请确认', '确认删除吗?', function(btn, text) {
					if (btn == 'yes') {
						...
						showWaitMsg();
						Ext.Ajax.request({
									url : 'integrateDemo.xzb?reqCode=deleteSfxm',
	...
	function printCatalog1() {
		...
		Ext.Ajax.request({
					url : 'integrateDemo.xzb?reqCode=buildReportDataObject',
