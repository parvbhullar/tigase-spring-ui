...
Ext.onReady(function() {
	...
	var store = new Ext.data.Store({
				proxy : new Ext.data.HttpProxy({
							url : 'excelReportDemo.xzb?reqCode=queryCatalogs4Export'
						}),
	...
	var grid = new Ext.grid.GridPanel({
		...
		tbar : [new Ext.form.TextField({
					...
					handler : function() {
						exportExcel('excelReportDemo.xzb?reqCode=exportExcel');
					}
				}, '-', {
					text : '导出(复杂表头)',
					tooltip : '以仿Ajax方式导出,界面无刷新',
					iconCls : 'page_excelIcon',
					handler : function() {
						exportExcel('excelReportDemo.xzb?reqCode=exportExcel2');
					}
				}],
		bbar : bbar
	});
