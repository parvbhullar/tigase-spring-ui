...
Ext.onReady(function() {
	...
	var deptTree = new Ext.tree.TreePanel({
				loader : new Ext.tree.TreeLoader({
							baseAttrs : {},
							dataUrl : './user.xzb?reqCode=departmentTreeInit'
						}),
				root : root,
				title : '',
				applyTo : 'deptTreeDiv',
	...
	var store = new Ext.data.Store({
				proxy : new Ext.data.HttpProxy({
							url : './user.xzb?reqCode=queryUsersForManage'
						}),
	...
	function saveUserItem() {
		...
		addUserFormPanel.form.submit({
					url : './user.xzb?reqCode=saveUserItem',
					waitTitle : '提示',
					method : 'POST',
					waitMsg : '正在处理数据,请稍候...',
				...
				.confirm(
						...
						function(btn, text) {
							if (btn == 'yes') {
								...
								showWaitMsg();
								Ext.Ajax.request({
									url : './user.xzb?reqCode=deleteUserItems',
	...
	function userGrantInit(userid, deptid, usertype) {
		...
		var selectRoleTab = new Ext.Panel({
			...
			autoLoad : {
				url : './user.xzb?reqCode=userGrantInit',
				scripts : true,
		...
		var selectMenuTab = new Ext.Panel({
			...
			// iconCls: 'user_femaleIcon',
			autoLoad : {
				url : './user.xzb?reqCode=selectMenuInit',
				scripts : true,
				text : parent.PAGE_LOAD_MSG,
				params : {
					userid : userid,
