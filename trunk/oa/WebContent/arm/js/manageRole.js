...
Ext.onReady(function() {
	...
	var deptTree = new Ext.tree.TreePanel({
				loader : new Ext.tree.TreeLoader({
							baseAttrs : {},
							dataUrl : './role.xzb?reqCode=departmentTreeInit'
						}),
				root : root,
				title : '',
				applyTo : 'deptTreeDiv',
	...
	var store = new Ext.data.Store({
				proxy : new Ext.data.HttpProxy({
							url : './role.xzb?reqCode=queryRolesForManage'
						}),
	...
	function saveRoleItem() {
		...
		addRoleFormPanel.form.submit({
					url : './role.xzb?reqCode=saveRoleItem',
					waitTitle : '提示',
					method : 'POST',
					waitMsg : '正在处理数据,请稍候...',
				...
				.confirm(
						...
						function(btn, text) {
							if (btn == 'yes') {
								...
								showWaitMsg();
								Ext.Ajax.request({
									url : './role.xzb?reqCode=deleteRoleItems',
	...
	function update() {
		addRoleFormPanel.form.submit({
					url : './role.xzb?reqCode=updateRoleItem',
					waitTitle : '提示',
					method : 'POST',
					waitMsg : '正在处理数据,请稍候...',
					success : function(form, action) {
	...
	function roleGrantInit(roleid, deptid, roletype) {
		...
		var operatorTab = new Ext.Panel({
			...
			autoLoad : {
				url : './role.xzb?reqCode=operatorTabInit',
				scripts : true,
		...
		var selectUserTab = new Ext.Panel({
			...
			// iconCls:'chart_organisationIcon',
			autoLoad : {
				url : './role.xzb?reqCode=selectUserTabInit',
				scripts : true,
				text : parent.PAGE_LOAD_MSG,
				params : {
					roleid : roleid,
		...
		var managerTab = new Ext.Panel({
			...
			autoLoad : {
				url : './role.xzb?reqCode=managerTabInit',
				scripts : true,
				text : parent.PAGE_LOAD_MSG,
				params : {
					roleid : roleid,
