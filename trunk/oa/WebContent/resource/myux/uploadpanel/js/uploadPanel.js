...
UploadPanel = Ext.extend(Ext.Panel, {
	...
	constructor : function(config) {
		...
		this.fileList = new Ext.grid.GridPanel({
			...
			ds : new Ext.data.SimpleStore({
				fields : ['fileId', 'fileName', 'fileSize', 'fileType', 'fileState','docfileid','docfilename','docdirname']
			}),
	...
	onUploadSuccess : function(file, serverData) {
		...
		if (Ext.util.JSON.decode(serverData).success) {
			...
			record.set('docfileid', Ext.util.JSON.decode(serverData).fileid);
			record.set('docfilename', Ext.util.JSON.decode(serverData).filename);
			record.set('docdirname', Ext.util.JSON.decode(serverData).dirname);
			Ext.getCmp('attaId').setValue(Ext.util.JSON.decode(serverData).fileid);
			$("#docAttaName").val(Ext.util.JSON.decode(serverData).filename);
			$("#docDirName").val(Ext.util.JSON.decode(serverData).dirname);
			//console.info("filename="+Ext.util.JSON.decode(serverData).filename+';$("#docAttaName")='+$("#docAttaName").val());
			record.commit();
		}
