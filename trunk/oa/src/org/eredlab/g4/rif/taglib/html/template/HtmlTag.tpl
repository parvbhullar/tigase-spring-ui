...
<head>
<title>${title}</title>
<meta http-equiv="keywords" content="雪中 豹">
<meta http-equiv="content-type" content="text/html; charset=UTF-8">
<meta http-equiv="description" content="It's Based eRedG4！copyRight(c) eRedLab">
<meta http-equiv="pragma" content="no-cache">  
<meta http-equiv="cache-control" content="no-cache, must-revalidate">  
<script src="${contextPath}/resource/jquery/jquery-1.3.2.min.js" type="text/javascript"></script>
<script src="${contextPath}/resource/jquery/jquery.form.js" type="text/javascript"></script>
<script src="${contextPath}/resource/jquery/json2.js" type="text/javascript"></script>
		...
		if (response.status == ajaxErrCode) {
			setTimeout(function(){
				...
				top.Ext.MessageBox.alert('提示', '由于长时间未操作,空闲会话已超时;您将被强制重定向到登录页面!', function() {
					parent.location.href = webContext + '/login.xzb?reqCode=init';
				});
			},200);
		}
  });
...
window.onload=function(){
  if(parent.userid != '${userInfo.getUserid()}'){
     top.Ext.MessageBox.alert('提示', '这是一个非法请求或者您的会话连接由于在其它窗口上被注销而失效,系统将把您强制重定向到登录界面.', function() {
		parent.window.location.href = webContext + '/login.xzb?reqCode=init';
	 });
  }
};
#end
