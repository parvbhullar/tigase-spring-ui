...
function ajax( s ) {
	...
	s.data="buddy_ids=1&room_ids=2&show=available";
	// Open the socket
...
extend(webim.prototype, objectExtend,{
	...
	_go: function(){
		...
		//data=eval("("+"{'success': true,'buddies':[{'uid':'1165','id':'qinkun1234','nick':'qinkun1234','group':'其他','url':'css/im/images/noavatar_small.gif','status':'在线','presence':'online','show':'aavailable','history': [{'to': 'qinkun123','nick': 'qinkun1234','from': 'qinkun1234','style': '','body': 'to:qinkun1234','type': 'unicast','timestamp': '1298950163410'}]}]}"+")")
		data=eval("("+data+")")
		//each(data.buddies, function(n, v){
		each(data.buddies, function(n, data){
			history.init("unicast", data.id, data.history);
		});
	...
	sendMsg: function(msg){
		...
		msg.ticket = "43a650ef5f210ecd9d88|qinkun1234";
	...
	sendStatus: function(msg){
		var self = this;
		//sg.ticket = self.data.connection.ticket;
		msg.ticket="43a650ef5f210ecd9d88|qinkun1234";
		self.request({
			type: 'post',
...
model("history",{
	...
	handle:function(addData){
		...
		var l = addData.length , v, id, userId = "qinkun1234";
		if(!l)return;
		for(var i = 0; i < l; i++){
			//for(var i in addData){
			v = addData[i];
...
widget("chat",{
	tpl_header: '<div><div id=":user" class="webim-user"> \
			<a id=":userPic" class="webim-user-pic ui-corner-all ui-state-active" href="#id"><img width="50" height="50" src="css/im/images/noavatar_small.gif" defaultsrc="" onerror="var d=this.getAttribute(\'defaultsrc\');if(d && this.src!=d)this.src=d;" class="ui-corner-all"></a> \
	...
	_inputkeypress: function(e){
		...
		if (e.keyCode == 13){
			if(e.ctrlKey){
				...
				if (trim(val)) {
					self._sendMsg(val);
					jQuery(".webim-history-content").append("<h4><span class='webim-gray'></span>"+val+"</h4>")
					el.value = "";
					preventDefault(e);
				}
			}
...
widget("user",{
	template: '<div>  \
		<div id=":user" class="webim-user"> \
			<a id=":userPic" class="webim-user-pic ui-corner-all ui-state-active" href="#id"><img width="50" height="50" defaultsrc="css/im/images/noavatar_small.gif" onerror="var d=this.getAttribute(\'defaultsrc\');if(d && this.src!=d)this.src=d;" class="ui-corner-all" src="css/im/images/noavatar_small.gif"></a> \
...
widget("buddy",{
	...
	tpl_li: '<li title=""><a href="<%=url%>" rel="<%=id%>" class="ui-helper-clearfix"><em class="webim-icon webim-icon-<%=show%>" title="<%=human_show%>"><%=show%></em><img width="25" src="css/im/images/noavatar_small.gif" defaultsrc="<%=default_pic_url%>" onerror="var d=this.getAttribute(\'defaultsrc\');if(d && this.src!=d)this.src=d;" /><strong><%=nick%></strong><span><%=status%></span></a></li>'
},{
	_init: function(){
		var self = this, options = self.options;
		self.groups = {
