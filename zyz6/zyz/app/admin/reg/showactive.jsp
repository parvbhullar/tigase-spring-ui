	...
	<div style="background: none repeat scroll 0% 0% rgb(255, 255, 255);" class="contect">
	  ...
	  <div class="newsbg3">
	     <table width="100%"  cellspacing="0" border="0" class="table2_border">
			  <tbody><tr>
						<td height="56" background="img/bg3.jpg" align="center" colspan="6"><font color="#ff3333" size="3"><strong>活动列表</strong></font></td>
			  </tr>
			  <tr align="center">
				...
				<td width="143" bgcolor="#fbe8cf">操作</td>
			  </tr>
			  <c:forEach var="active" items="${activeList}" varStatus="status">
			  <tr align="center">
			  	<td height="26" align="center">${status.count + (page-1)*size }</td>
				<td align="center">${active.acttitle }</td>
				<td align="center">${active.startdate }</td>
				<td align="center">${active.enddate }</td>
				<td>
					&nbsp;
					<c:if test="${active.join==true}">
						已经加入
					</c:if>
				</td>
				<td align="center">	&nbsp;
					<c:if test="${active.join==false}">
						<a href="confirmActive.action?activeId=${active.actid}&dn=${postulant.dn}">参加</a>
					</c:if>						
		...
		<script type="text/javascript">
			...
			{
				document.location.href='showactive.action?page='+page;
			}
		</script>
