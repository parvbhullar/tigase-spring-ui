...
function setButtonDisable(b, c) {
    ...
    if (d) if (!c && arguments.length == 2) window.setTimeout("setButtonDisable('" + b + "',false,0);", 100);
        ...
        var e = "";
        if (d.className.indexOf("minbtn-wrap") > -1) e = "minbtn-disable";
        else if (d.className.indexOf("btn-wrap") > -1) e = "btn-disable";
...
function initMiniEditor(b, c, d) {
    var e = null,
    f = {};
    if (typeof b == "string") if (d != true) f.maxHeight = max.coor.height($$(b)[0]);
    f.buttonList = c || editorToolBar.full;
    f.iconsPath = root + "/max-assets/nicedit/nicEditorIcons.gif";
    if (typeof b == "string") {
        if (b = $$(b)[0]) e = (new nicEditor(f)).panelInstance(b).nicInstances[0]
...
function initDisplay(b, c) {
    ...
    _display = function(e, f) {
        ...
        for (h = 0; h < f.length; h++) {
            g = f[h];
            if (g.value.toLowerCase() == d) if (typeof g.id == "string") $(g.id).style.display = g.display ? "": "none";
            else for (i = 0; i < g.id.length; i++) $(g.id[i]).style.display = g.display ? "": "none"
        }
...
var BBSMAX = {
    AvatarDefault: root + "/max-assets/avatar/avatar_120.gif",
    ImageDefault: root + "/max-assets/images/notfound.gif",
    CodeUrl: "validatecode.aspx?type=",
    CodeLoadingImage: root + "/max-assets/images/loading.gif",
    MessageSoundPath: root + "/max-assets/sound-msg/",
    SpaceImage: root + "/max-assets/images/blank.gif",
    NetExistsExt: /^(accdb|as|asm|aspx|avi|bmp|c|cfc|chm|cpp|cs|css|db|default|dll|doc|docx|eip|fla|flv|gif|h|html|jpg|js|mdb|mp3|mpeg|mpg|msi|pdf|php|png|ppt|pptx|psd|rar|sln|swf|tif|txt|vb|vs|wav|wma|wmv|xls|xlsx|xml|zip)$/i,
    NetExistsWidth: 48,
    NetExistsHeight: 48,
    NetDefaultWidth: 70,
    NetDefaultHeight: 70,
    ImagePreviewBg: "/max-assets/images/imagesPreview_percent_bg.gif"
};
    ...
    for (var f = (new Date).getMilliseconds().toString(), g = 0, h = 0; h < d.length; h++) {
        ...
        if (i && i.length) for (var j = 0; j < i.length; j++) {
            ...
            if (k.type == "submit") {
                ...
                if (!k.id) k.id = String.format("max_submit_{0}", (new Date).getTime());
                var q = k.id;
                if (e > 0) {
                    ...
                    q = String.format(n + 't=$("' + q + '"); if(t.disabled) return false; t.disabled="disabled";addCssClass(t,"button-disable"); p.panel.style.cursor="wait";' + l);
                    if (g == 0) {
                        m.defaultButton = k;
...
DynamicTable.prototype.insertRow = function(b) {
    ...
    var h = this,
    i = this.maxKey;
    addHandler($(String.format(this.deleteTrigger, this.maxKey)), "click",
    function() {
        h.deleteRow(i)
    });
    b && b(this.maxKey);
...
function initColorSelector(b, c) {
    ...
    function e() {
        ...
        if (i == "") i = "ffffff";
        var j = root + "/max-assets/javascript/colorboard.html?color=" + i;
        window.setTimeout(function() {
...
function delElement(b, c) {
    if (typeof b.toString().toLowerCase() == "string") b = $(b);
    hide(b,
    function() {
...
function onCtrlEnter(b, c) {
    for (var d = typeof b != "object" ? $(b) : b; d && d.nodeName.toLowerCase() != "form";) d = d.parentNode;
...
createBackgroundFrame = function() {},
timer = function(b, c) {
    this.counter = 0;
    this.interval = b;
    this.ontick = c;
...
ajaxSubmit = function(b, c, d, e, f, g) {
    ...
    if (!d || d == "") d = "*";
    if (d != null) b += hasQuery(b) ? "&_max_ajaxids_=" + d: "?_max_ajaxids_=" + d;
    if (!f) return true;
    ...
    function(h) {
        ...
        h = null;
        for (m in i) if (m == "_max_ajaxresult_") h = eval("(" + i[m] + ")");
        else {
...
var popupBase = function(b, c, d) {
    this.triggers = [];
    this.list = typeof b == "object" ? b: $(b);
    this.auto = d;
    if (c instanceof Array) for (b = 0; b < c.length; b++) {
        d = $(c[b]);
        d != null && this.triggers.push(d)
    } else c instanceof Object ? this.triggers.push(c) : this.triggers.push($(c))
};
...
var popup = function(b, c, d, e, f) {
    ...
    b.position = "absolute";
    b.zIndex = 50;
    if (this.triggers.length > 0) if (elementInDialog(this.triggers[0])) b.zIndex = 52;
    for (b = 0; b < this.triggers.length; b++) {
        c = this.triggers[b];
...
var elementInDialog = function(b) {
    b = b.parentNode;
    for (var c = 0; b && c < 20;) {
        if (b.id && b.id.indexOf("mx_dialog_") > -1) return true;
        c++
    }
    return false
};