...
$(document).ready(function() {
    ...
    $("#wapDraftSave").click(function() {
        ...
        saveWapPage(
          function(data) {
              if (data.flag == "1" && $("#draftStatus").val()!="1"&&$("#submitStatus").val()!="1" ) {
                  alert("修改成功");
              }
              else {
                  if (data.wapId != null && data.wapId != "0") {
                      if ($("#draftStatus").val()!="1") {
                          alert("保存成功");
                      }
                      $("#wapId").val(data.wapId);
                  }
              }
              if ($("#draftStatus").val() != "1" && $("#submitStatus").val() != "1") {
                $("#closeWap").click();
    ...
    $("#saveAd").click(function(e) {
        ...
        if ($("#draftStatus").val()=="1") {
            window.location.href = "ad.do?action=adList&adGroupId=" + $("#adGroupId").val() + "&campaignId=" + $("#campaignId").val();
            return false;
        }
    });
    //提交广告
    ...
    $("#pic01").click(function() {
      if($('#isNeedNotifyWhenChange2Wap').val()=='true'){
        if(!confirm('之前广告内容将被清空，您确定要切换至wap页广告么？'))
          return false;
      }
      $('#isNeedNotifyWhenChange2Wap').val("false");
        $("#previewNavigation").hide();
        $("#delBackgroundPhoto").hide();
        $(".spanAdNavigation").parent().parent().hide();
        $(".spanAdWap").parent().parent().hide();
        $(".spanAdWap").html($(this).attr("rel")).parent().parent().show();
        $(".spanAdRich").hide();
        $("#adType").val(1);//广告类型1wap
        $("#pic01").addClass("now");
        $("#pic02").removeClass("now");
    ...
    con20.hide();
        $(".con22").hide();
        $("#adBackground").hide();
        $('div[class=demoCon22 con22]').remove();
        getFree();
    });
    //单页面
    ...
    $("#pic03").click(function() {
      $('#isNeedNotifyWhenChange2Wap').val("true");
        ...
        $(".spanAdWap").parent().parent().hide();
        $(".spanAdRich").show();
        $(".spanAdNavigation").html($(this).attr("rel")).parent().parent().show();
        $("#adType").val(3);//广告类型3普通按钮
        $("#adBackground").show();
        //removeAllBox();

        /**
         * 修改使单页面换成 导航页面不再保存淘宝直链数据   penghan
         */
        //如果原来选择的是单页面,且商品选择了淘宝直链的话,就清空
        if($("#pic02").attr("class")=="now" && $(".showType1RichLabel").attr("rev")=="6"){
            $(".con22").remove();
        }
        if($("#pic02").attr("class")=="now" && ($(".showType1RichLabel").attr("rev")=="7"||$(".showType1RichLabel").attr("rev")=="8")){
            $(".con22").remove();
        }
        $("#pic03").addClass("now");
        $("#pic02").removeClass("now");
        ...
        $("#con21").hide();
    var con20=$("#con20");
        con20.find("select").removeAttr("style");
    con20.show();
        createChildPage(3);
        $(".con22").show();
        getFree();
...
function checkPage4Save(target) {
          ...
          if ($("#richMediaTitle" + uiId).val().trim() == "") {
              if ($("#draftStatus").val()!="1") {
                  alert("请填写页面名称");
                  return false;
              }
  ...
  function checkBanner4Save() {
        ...
        var errMagess=""
            ...
            var bannerTextTrim=bannerText.replaceAll("<br>","").replaceAll("<br/>","");
